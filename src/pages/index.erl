...
main() -> #dtl{file=index,
               app=uu,
               bindings=[{news,fetch(news,lang(),fun update_links/1)},
                         {body,body()},
                         {interviews,fetch(interviews,lang(),fun update_links/1)}]}.
...
fetch(Name, Lang, Fun) ->
    LangList = wf:to_list(Lang),
    List = lists:flatten([ begin
       case string:tokens(F,".") of
         [D,A,L,"txt"] when L == LangList -> Fun([D,A,L]);
                                       _  -> [] end
...
update_links([D,A,L]) ->
    ...
    article_link({Date,A,Language,HTML}).
