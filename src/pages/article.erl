...
main() ->
   Author = wf:to_list(wf:q(author)), Date  = wf:to_list(wf:q(date)),
   Language = wf:to_list(wf:q(locale)),
   [Name,Surname] = string:tokens(Author,"-"),
   {_,#user{name=FullName,city=City,profession=Profession,photo=Photo}} = uu_people:lookup({Author,wf:atom([Language])}),
